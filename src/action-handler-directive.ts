...
if (!customElements.get('action-handler-animated-weather')) {
  customElements.define('action-handler-animated-weather', ActionHandler)
}

const getActionHandler = (): ActionHandler => {
  const body = document.body
  if (body.querySelector('action-handler-animated-weather')) {
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    return body.querySelector('action-handler-animated-weather')!
  }

  const actionhandler = document.createElement('action-handler-animated-weather')
