...
export class WeatherVisualEngine {
  private readonly canvasBg: HTMLCanvasElement
  private readonly canvasScene: HTMLCanvasElement
  private readonly canvasFx: HTMLCanvasElement
  private readonly ctxBg: CanvasRenderingContext2D
  private readonly ctxScene: CanvasRenderingContext2D
  private readonly ctxFx: CanvasRenderingContext2D
  ...
  constructor (bg: HTMLCanvasElement, scene: HTMLCanvasElement, fx: HTMLCanvasElement) {
    ...
    if (!ctxBg || !ctxScene || !ctxFx) {
      throw new Error('Could not get 2D context from canvas')
    }
    this.ctxBg = ctxBg
    this.ctxScene = ctxScene
    this.ctxFx = ctxFx
  }
  ...
  private update (): void {
    ...
    this.particles.forEach(p => {
      if (p.type === 'rain') {
        ...
        if (p.y > this.h) { p.y = -(p.l ?? 10); p.x = rnd(0, this.w) }
      } else if (p.type === 'snow') {
        p.y += p.v
        ...
        p.x += (p.vx ?? 0)
        const fogRadius = p.r ?? 50
        if (p.x > this.w + fogRadius) p.x = -fogRadius
      } else {
        // sunny/floating/dust
  ...
  private drawSoftClouds (ctx: CanvasRenderingContext2D, colorStart: string, colorEnd: string): void {
    ctx.save()
    this.clouds.forEach(c => {
      const effectiveRadius = c.r
  ...
  private drawScene (ctx: CanvasRenderingContext2D): void {
    ...
    this.particles.forEach(p => {
      if (p.type === 'rain') {
        ...
        ctx.beginPath()
        ctx.moveTo(p.x, p.y)
        const slant = this.weather === 'pouring' ? -5 : -2
        ...
        const debrisRadius = p.r ?? 2
        ctx.fillRect(-debrisRadius, -debrisRadius / 2, debrisRadius * 2, debrisRadius)
        ctx.restore()
      } else if (p.type === 'fog') {
